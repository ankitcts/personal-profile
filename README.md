...
### A. Just the dashboard with your own data
...
3. Serve the folder (the dashboard loads `data/data.js` as a script, so a local HTTP server is required):
   ```sh
   python3 -m http.server 8000
   ```
   Then open <http://localhost:8000/spending-dashboard.html>.

### B. Auto-extract transactions from your Gmail

The repo includes a Python extractor that reads receipts directly from Gmail and writes `data/data.js` for you. Merchants are defined in a JSON catalog (`extractor/senders.json`) — adding a new one is a JSON edit, not a code change.

```sh
cd extractor
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
# follow extractor/README.md to set up Gmail OAuth credentials
python -m extractor.extract --since 2021-11-01
```

See **[`extractor/README.md`](extractor/README.md)** for the OAuth setup walkthrough and `senders.json` schema.
...
data/
  data.js                 ← your data + config (auto-generated by extractor, or hand-edited)
  data.example.js         ← template / placeholder
extractor/                ← optional: Gmail → data.js pipeline
  senders.json            ← merchant catalog (edit to add merchants)
  extract.py              ← CLI entry point
  README.md               ← OAuth setup + schema docs
README.md
```
